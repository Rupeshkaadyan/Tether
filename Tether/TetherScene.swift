...
struct TetherScene: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                // which is a property of the SCENE, not the clock. Jungle has
                // a dark sky and no stars; Night and Aurora have both.
                ...
                // Far layer.
                SceneSilhouetteShape(kind: palette.silhouette, depth: 0)
                    .fill(farRidge)
                    .frame(height: h)

                // Near layer.
                SceneSilhouetteShape(kind: palette.silhouette, depth: 1)
                    .fill(nearRidge)
                    .frame(height: h)
    ...
    // MARK: Palette
    //
    // Driven by the SCENE, not the time of day. Recolouring one landscape by
    // clock is what produced a blue sky over a green jungle: the body was
    // Jungle and the header was "afternoon". A scene now owns its own sky,
    // light, ridges and water, and `scene.header` holds all of it.
    //
    // `timeOfDay` is kept only for the silhouette decision below, where the
    // scene does not already imply one.

    private var palette: SceneHeader { scene.header }

    private var sky: [Color] { palette.sky }
    private var orb: Color { palette.orb }
    private var farRidge: Color { palette.far }
    private var nearRidge: Color { palette.near }
    private var water: Color { palette.water }
}
